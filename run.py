...
def run():
    ...
    train_data_iter = DataLoader(train_dataset,batch_size=64,shuffle=True) #
    ...
    net = HLSTM(True,train_dataset.pretrained_emb,train_dataset.token_size).to(device)
    ...
    #     #print("pred: ", pred, "label: ", label)
        ...
        with torch.no_grad():
            ...
            for face_test,voice_test,text_test,ans_test in test_data_iter:
                ...
                result = net(face_test,voice_test,text_test) #batch, 2
                lo = criteon(result.squeeze(1),ans_test)
                total_loss+=lo
                
                result = torch.gt(torch.sigmoid(result),0.5).int()         # BCELOSS
